...
if __name__ == " __main__":
    ...
    filename = "4_add_twelve_nosphs.xlsx"
    ...
    sc = sc_ref.clone(model, "general_nosphs", keep_solution=False)
    # sc.check_out()
    ...
    for storage in range(1, num_storage + 1):
        ...
        tec_list = [f"dam_hydro_{storage}", f"dam_sphs_{storage}"]
        par_list = ["fix_cost", "inv_cost"]
        for parname in par_list:
            df = sc.par(parname, {"technology": tec_list})
            sc.remove_par(parname, df)
